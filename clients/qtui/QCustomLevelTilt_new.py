...
class QCustomLevelTilt(QtGui.QFrame):
    onNewValues = QtCore.pyqtSignal()
    def __init__(self, title, channelNames, channelRanges, parent=None):
        QtGui.QWidget.__init__(self, parent)
        self.setFrameStyle(0x0001 | 0x0030)
        self.makeLayout(title, channelNames)
        
    def makeLayout(self, title, channelNames):
        layout = QtGui.QGridLayout()
        #labels
        title = QtGui.QLabel(title)
        title.setFont(QtGui.QFont('MS Shell Dlg 2',pointSize=16))
        labelLeft = QtGui.QLabel(channelNames[0])
        labelRight = QtGui.QLabel(channelNames[1])
        levelLabel = QtGui.QLabel("Level")
        tiltLabel = QtGui.QLabel("Tilt")
        for label in [title, labelLeft, labelRight, levelLabel, tiltLabel]:
            label.setAlignment(QtCore.Qt.AlignCenter)
        layout.addWidget(title,      0, 0, 1, 4)
        layout.addWidget(labelLeft,  1, 0, 1, 1)
        layout.addWidget(labelRight, 1, 3, 1, 1)
        layout.addWidget(levelLabel, 2, 1, 1, 1)
        layout.addWidget(tiltLabel,  4, 1, 1, 1)
        #editable fields
        self.sliderLevel = QtGui.QSlider(QtCore.Qt.Horizontal)
        self.sliderTilt = QtGui.QSlider(QtCore.Qt.Horizontal)
        self.spinLevel = QtGui.QDoubleSpinBox()
        self.spinTilt = QtGui.QDoubleSpinBox()
        self.valueLeft = QtGui.QDoubleSpinBox()
        self.valueRight = QtGui.QDoubleSpinBox()
        self.spinLeft = QtGui.QDoubleSpinBox()
        self.spinRight = QtGui.QDoubleSpinBox()
        for spin in [self.spinLevel, self.spinTilt, self.spinLeft, self.spinRight]:
            spin.setFont(QtGui.QFont('MS Shell Dlg 2',pointSize=16))
        layout.addWidget(self.sliderLevel, 3, 1, 1, 1)
        layout.addWidget(self.sliderTilt,  5, 1, 1, 1)
        layout.addWidget(self.spinLevel,   2, 2, 2, 1)
        layout.addWidget(self.spinTilt,    4, 2, 2, 1)
        layout.addWidget(self.spinLeft,    2, 0, 4, 1)
        layout.addWidget(self.spinRight,   2, 3, 4, 1)
...
if __name__=='__main__':
    ...
    icon = QCustomLevelTilt('Control',['A','B'],[(0.0,100.0), (50.0, 150.0)])
    icon.show()
    app.exec_()