...
def micro_model(params, x):
    ...
    for n in np.arange(-50,51,1):
        flour_temp1 = (bessel.jv(n,beta)**2+grad*(bessel.jv(n-1,beta)-bessel.jv(n+1,beta))**2)/((x+n*Omega-4.0*B/3.0)**2+(gamma/2.0)**2)
        flour_temp2 = (bessel.jv(n,beta)**2+grad*(bessel.jv(n-1,beta)-bessel.jv(n+1,beta))**2)/((x+n*Omega-2.0*B/3.0)**2+(gamma/2.0)**2)
        flour_temp3 = (bessel.jv(n,beta)**2+grad*(bessel.jv(n-1,beta)-bessel.jv(n+1,beta))**2)/((x+n*Omega+4.0*B/3.0)**2+(gamma/2.0)**2)
        flour_temp4 = (bessel.jv(n,beta)**2+grad*(bessel.jv(n-1,beta)-bessel.jv(n+1,beta))**2)/((x+n*Omega+2.0*B/3.0)**2+(gamma/2.0)**2)
...
dv.cd('','Experiments','BareLineScan','2013Nov16','1629_27')
dv.open(3)
data2=dv.get().asarray

data2_y = data2[:,1]/39.0
data2_yerr = np.sqrt(data2[:,1])/39.0
...
params.add('amplitude', value = 46583, vary = False)
params.add('gamma', value = 24.0,vary = False)
params.add('offset', value = 0.016961)
params.add('beta', value = 1.21)
params.add('Omega', value = 30.704, vary = False)
params.add('B', value = 4.32)
params.add('center', value = 227, vary = False)
params.add('grad', value = 0.0)
