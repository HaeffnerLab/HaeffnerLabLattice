...
avar = []
allan_error_bar = []
cf = 3 ## overlapping factor #choose 1 for non-overlapping allan
...
for bin_size in np.linspace(0.0,max(time)/4.0,49):
    ...
    for j in range(0,cf):
        ...
        for i in range(0,int(np.floor(max(time-time_offset)/bin_size))):
    ...
    allan_error_bar.append(avar_result*0.5*np.sqrt(1/(M)))
