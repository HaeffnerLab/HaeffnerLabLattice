...
#plot parity
#dv.cd(['','Experiments','Ramsey2ions_ScanGapParity','2014Jan29','1756_34'])
dv.cd(['','Experiments','Ramsey2ions_ScanGapParity','2014Feb10','1842_06'])
dv.open(2)
data = dv.get().asarray
x3 = data[:,0]
y3 = data[:,1]
...
dv.cd(['','Experiments','Ramsey2ions_ScanGapParity','2014Feb10','1845_52'])
dv.open(2)
data = dv.get().asarray
x4 = data[:,0]
y4 = data[:,1]

x_hot = np.concatenate((x3[:-6],x4[:-4]),axis=0)/1000000
y_hot = np.concatenate((y3[:-6],y4[:-4]),axis=0)

pyplot.plot(x_hot, y_hot,'or')

pyplot.axis([0,0.2,-1.1,1.1])
