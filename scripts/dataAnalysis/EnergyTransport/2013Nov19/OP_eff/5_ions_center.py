...
date = '2013Nov18'; comparing_flops = ('1347_35', '1352_43'); repetitions = 100.;
...
dv.open(1)
data = dv.get().asarray
times_pumped, a,b,probs_pumped,c,d = data.transpose()
dv.cd( ['','Experiments','RabiFlopping',date,original_name])
dv.open(1)
data = dv.get().asarray
times_original, a,b,probs_original,c,d = data.transpose()

times_pumped = times_pumped[:-12]
probs_pumped = probs_pumped[:-12]
print times_original.size
print times_pumped.size
