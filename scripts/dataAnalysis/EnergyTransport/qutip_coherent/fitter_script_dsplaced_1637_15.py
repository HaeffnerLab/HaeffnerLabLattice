...
params.add('excitation_scaling', value = 1, vary = False)
...
'''
guess_evolution = flop.compute_evolution_thermal(params['nbar'].value , params['detuning'].value, params['time_2pi'].value, detailed_times - offset_time )
...
fit_values = flop.compute_evolution_thermal(params['nbar'].value , params['detuning'].value, params['time_2pi'].value, detailed_times - offset_time )
lmfit.report_errors(params)
'''
make the plot
'''
pyplot.figure()
pyplot.plot(detailed_times, guess_evolution, '--k', alpha = 0.2, label = 'initial guess')
