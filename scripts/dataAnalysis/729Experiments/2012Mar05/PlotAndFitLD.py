...
class StateEvolution:
    ...
    def rabiflop(self,nbar,omega0,delta,sideband_order,t):
        n=self.n
        result=omega0/np.sqrt(omega0**2+delta**2)*np.sum(self.pnb(nbar,sideband_order,np.outer(n,np.ones_like(t)))*np.sin(np.outer(np.sqrt(self.omegaeff(omega0,sideband_order,n)**2+delta**2),t))**2,axis=0)
        return result
