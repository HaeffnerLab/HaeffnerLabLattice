...
pyplot.xlabel(r'$\frac{\Omega t}{2\pi}$',fontsize=22)
pyplot.ylim((0,1))
...
pyplot.plot(np.array(times),2.0*np.array(averages),'ko',label='Time-Averaged Qubit Distance')
pyplot.plot(fake_times*fake_f,discord,'k-',label='Discord between Qubit and Motion')
pyplot.plot(fake_times*fake_f,flop,'b-',label='Blue Sideband Rabi Flop, nbar = {:.2f}'.format(nbar))

pyplot.legend(loc=2,prop={'size':20})
pyplot.title('Time-Averaged Distance to Assess Quantum Discord',fontsize=30)
