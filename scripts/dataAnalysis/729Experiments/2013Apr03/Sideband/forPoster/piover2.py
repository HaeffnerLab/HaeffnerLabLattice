...
pyplot.plot(np.array(flop_x_axis)*timescale,flop_y_axis, 'ro',linewidth = 2.0)

yerrflop = np.sqrt((1-flop_y_axis)*flop_y_axis/(100.0*len(flop_files)))
pyplot.plot(np.array(flop_x_axis)*timescale, flop_y_axis,'ro',markersize = 8)
yerrdeph = np.sqrt((1-deph_y_axis)*deph_y_axis/(100.0*len(dephase_files)))
pyplot.plot(np.array(deph_x_axis)*timescale, deph_y_axis,'bo',markersize = 8)
pyplot.plot(np.array(deph_x_axis)*timescale,deph_y_axis, 'bs',linewidth = 2.0)
