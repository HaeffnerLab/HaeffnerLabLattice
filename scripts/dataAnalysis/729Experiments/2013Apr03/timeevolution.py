...
class time_evolution():#contains all relevant functions for thermal states, rabi flops and dephasing
    ...
    def p_thermal(self,nbar,nshift = True):
        n = self.n
    ...
    def discord_fluc(self,t,nbar,f_Rabi,delta_center,delta_variance,n_fluc=5.0):
        dis_list=[]
        i_list=np.linspace(-1,1,n_fluc)*np.exp(-np.linspace(-1,1,n_fluc)**2/2.0)/np.sqrt(2.0*np.pi)
        for i in i_list:
            dis_list.append(self.discord(t,nbar,f_Rabi,delta_center+delta_variance*i))
        result = np.sum(dis_list,axis=0)/float(n_fluc)
        return result
    ...
    def discord(self,t,nbar,f_Rabi,delta=0.0):
        p=self.p_thermal(nbar)
        def pge(t):
            return self.g_to_e_prob(t, f_Rabi, delta)
        def pgg(t):
            return self.g_to_g_prob(t, f_Rabi, delta)
        ones = np.ones_like(t)
        result=2.0*np.sum(np.outer(p**2,ones)*np.abs(pgg(t)*pge(t))**2,axis=0)
        return result