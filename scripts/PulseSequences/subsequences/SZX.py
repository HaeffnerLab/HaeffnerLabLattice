...
class szx_ramsey(pulse_sequence):
    ...
    def sequence(self):
        ...
        # carrier pi/2 pulse
        self.addDDS('729local', self.start + frequency_advance_duration, t_pi2, p.frequency, car_amp, p0)
        te = self.start + frequency_advance_duration + t_pi2 + buf
        
        # now bichromatic pulse
        self.addTTL('bichromatic_2', te, gate_duration + 2*buf)
        self.addDDS('729local', te + buf, gate_duration, p.frequency, szx_amp, p0)
        te = te + gate_duration + 2*buf

        # second carrier pulse
        self.addDDS('729local', te+buf, t_pi2, p.frequency, car_amp, p2)
...
class szx(pulse_sequence):
    ...
    def sequence(self):
        ...
        self.addDDS('729global', te , gate_duration, p.frequency, szx_amp)
