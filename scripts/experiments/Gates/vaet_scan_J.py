...
class vaet_scan_J(experiment):
    ...
    def setup_sequence_parameters(self):
        ...
        minim,maxim,steps = vaet.ms_scan_amplitude
    ...
    def setup_data_vault(self):
        ...
        ds=self.dv.new('VAET Scan J {}'.format(datasetNameAppend),[('Excitation', 'us')], dependents , context = self.save_context)
    ...
    def run(self, cxn, context):
        ...
        for i,amplitude in enumerate(self.scan):
            should_stop = self.pause_or_stop()
            if should_stop: break
            excitation = self.get_excitation_crystallizing(cxn, context, amplitude)
            if excitation is None: break 
            submission = [amplitude['dBm']]
            submission.extend(excitation)
            self.dv.add(submission, context = self.save_context)
            self.update_progress(i)
    
    def get_excitation_crystallizing(self, cxn, context, amplitude):
        # right now don't crystallize because I'm not sure if it works
        excitation = self.do_get_excitation(cxn, context, amplitude)
        #if self.parameters.Crystallization.auto_crystallization:
    ...
    def do_get_excitation(self, cxn, context, amplitude):
        self.load_frequency()
        self.parameters['MolmerSorensen.amplitude'] = amplitude
