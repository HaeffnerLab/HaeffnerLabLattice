...
from excitations import MS_parity_flop 
...
class ms_scan_time_parity(experiment):
    
    name = 'MS_Scan_time_parity'
    trap_frequencies = [
    ...
    def all_required_parameters(cls):
        ...
        parameters = parameters.union(set(MS_parity_flop.all_required_parameters()))
    ...
    def initialize(self, cxn, context, ident):
        self.ident = ident
        self.excite = self.make_experiment(MS_parity_flop)
    ...
    def setup_sequence_parameters(self):
        ...
        minim,maxim,steps = gate.wait_time
    ...
    def run(self, cxn, context):
        ...
        for i,wait_time in enumerate(self.scan):
            should_stop = self.pause_or_stop()
            if should_stop: break
            parity = self.get_parity_crystallizing(cxn, context, wait_time)
            if parity is None: break 
            submission = [wait_time['us']]
            #submission.append(parity)
            submission.extend([parity])
            self.dv.add(submission, context = self.save_context)
            self.update_progress(i)
    
    def get_parity_crystallizing(self, cxn, context, wait_time):
        # right now don't crystallize because I'm not sure if it works
        parity = self.do_get_parity(cxn, context, wait_time)
        #if self.parameters.Crystallization.auto_crystallization:
    ...
    def do_get_parity(self, cxn, context, wait_time):
        self.load_frequency()
        self.parameters['MolmerSorensen.wait_time'] = wait_time
...
if __name__ == '__main__':
    ...
    exprt = ms_scan_time_parity(cxn = cxn)
