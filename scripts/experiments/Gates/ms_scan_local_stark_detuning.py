...
class ms_scan_local_stark_detuning(experiment):
    
    name = 'MS_ScanLocalStarkDetuning'
    trap_frequencies = [
    ...
    def all_required_parameters(cls):
        ...
        #parameters.remove(('LocalStarkShift', 'scan'))
        return parameters
    ...
    def setup_sequence_parameters(self):
        ...
        minim,maxim,steps = self.parameters.LocalStarkShift.scan_detuning
    ...
    def setup_data_vault(self):
        ...
        self.dv.add_parameter('Window', ['Local AC Stark Detuning'], context = self.save_context)
        #self.dv.add_parameter('plotLive', True, context = self.save_context)
        if self.grapher is not None:
            self.grapher.plot_with_axis(ds, 'ms_local_stark_detuning', self.scan)
    ...
    def run(self, cxn, context):
        ...
        for i,det in enumerate(self.scan):
            ...
            excitation = self.get_excitation_crystallizing(cxn, context, det)
    ...
    def get_excitation_crystallizing(self, cxn, context, det):
        # right now don't crystallize because I'm not sure if it works
        excitation = self.do_get_excitation(cxn, context, det)
        #if self.parameters.Crystallization.auto_crystallization:
    ...
    def do_get_excitation(self, cxn, context, det):
        self.load_frequency()
        self.parameters['LocalStarkShift.detuning'] = det
...
if __name__ == '__main__':
    ...
    exprt = ms_scan_local_stark_detuning(cxn = cxn)
