...
from excitation_ramsey_spectrum import excitation_ramsey_spectrum
...
class blue_heat_scan_pulse_phase_ramsey(experiment):
    
    name = 'Blue Heat Scan Pulse Phase Ramsey'
    trap_frequencies = [
    ...
    def initialize(self, cxn, context, ident):
        ...
        self.square_pulse_generator.modulation_state(True)
        self.square_pulse_generator.psk_modulation_source('external')
        self.square_pulse_generator.phase(WithUnit(0,'deg'))
        self.square_pulse_generator.modulation_type('PSK')
    ...
    def setup_sequence_parameters(self):
        ...
        minim,maxim,steps = self.parameters.PulsedScanRamsey.scan_phase
    ...
    def run(self, cxn, context):
        ...
        for i,phase in enumerate(self.scan):
            ...
            excitation = self.get_excitation_crystallizing(cxn, context, phase)
    ...
    def do_get_excitation(self, cxn, context, phase):
        self.load_frequency()
        self.excite.set_parameters(self.parameters)
        self.square_pulse_generator.psk_phase(phase)
        excitation = self.excite.run(cxn, context)
        return excitation
    
    def finalize(self, cxn, context):
        self.save_parameters(self.dv, cxn, self.cxnlab, self.rabi_flop_save_context)
        self.square_pulse_generator.modulation_state(False)
...
if __name__ == '__main__':
    ...
    exprt = blue_heat_scan_pulse_phase_ramsey(cxn = cxn)
