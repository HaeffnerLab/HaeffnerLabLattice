...
class Parity_LLI_2_point_monitor(experiment):
    ...
    def initialize(self, cxn, context, ident):
        ...
        self.save_data_short_time = cxn.context()
    ...
    def setup_data_vault(self):
        ...
        self.dv.cd(directory_LLI ,True, context = self.save_data_short_time)
        datasetname_parity = 'LLI_data_new_short_time'
        dataset_parity_in_folder = self.dv.dir(context= self.save_data_short_time)[1]
        names = sorted([name for name in dataset_parity_in_folder if datasetname_parity in name])
        if names:
            #dataset with that name exist
            self.dv.open_appendable(names[0],context=self.save_data_short_time)
        else:
            #dataset doesn't already exist
            self.dv.new(datasetname_parity,[('Time', 'Sec')],[('Parity','Mirror','Probability'),('Parity','No Mirror','Probability'),('Parity','Average','Probability'),('Parity','Difference','Probability')],context=self.save_data_short_time)
            window_name = [datasetname_parity]
            self.dv.add_parameter('Window', window_name,context=self.save_data_short_time)
            self.dv.add_parameter('plotLive', True,context=self.save_data_short_time) 
