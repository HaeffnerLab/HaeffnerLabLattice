...
class ramsey_with_heating_and_cryst_scan_blue(experiment):
    
    name = 'Ramsey_with_Heating_Pulse_and_Cryst_Scan_Blue'
    ramsey_with_heating_required_parameters = [
                           ...
                           ('Dephasing_Pulses', 'scan_blue_heating_start_time'),
    ...
    def initialize(self, cxn, context, ident):
        ...
        minim_pwr,maxim_pwr,pwr_steps = self.parameters.Dephasing_Pulses.scan_blue_heating_power
        
        minim_phi,maxim_phi,phi_steps = self.parameters.Dephasing_Pulses.scan_phase #Ahmed

        minim_pwr = minim_pwr['dBm']; maxim_pwr = maxim_pwr['dBm']  #Ahmed
        minim_phi = minim_phi['deg']; maxim_phi = maxim_phi['deg']  #Ahmed

        self.scan_pwr = linspace(minim_pwr,maxim_pwr, pwr_steps)  #Ahmed
        self.scan_pwr = [WithUnit(pt_pwr, 'dBm') for pt_pwr in self.scan_pwr]  #Ahmed
    ...
    def data_vault_new_trace(self):
        ...
        window_name = ['Ramsey with Heating - Scan Blue Start']
        ...
        window_name = ['Dephasing, Contrast - Scan Blue Start']
    ...
    def run(self, cxn, context):
        ...
        # check that the blue pulse does no collide with the second ramsey pulse
        #if self.scan_t
        
        p.evolution_ramsey_time = self.parameters.RamseyDephase.pulse_gap                                                            
        ...
        print self.scan_pwr[-1].value
        print total_heating_time.value
        
        if (self.parameters.Heating.blue_heating_delay_before > (ramsey_time.value - total_heating_time.value)):
            print "The blue pulse will overlap with the second Ramsey pulse during this scan"
            print "Max possible value for scan.blue_heating_start_time", ramsey_time - total_heating_time
            return False
                                     
        
        #for i,interaction_duration in enumerate(self.scan_t):
        for i,interaction_pwr in enumerate(self.scan_pwr):
            ...
            #second_pulse_dur = self.max_second_pulse
            
            # the ramsey time is kept constant in this scan
            self.parameters.Heating.local_blue_heating_amplitude_397 = interaction_pwr
            ...
            for j,interaction_phase in enumerate(self.scan_phi):                
                ...
                if self.parameters.Heating.enable_heating_pulse:                                
                    ...
                    if min_ramsey_time.value < 0:
                        ...
                        print "397 power", self.parameters.Heating.local_blue_heating_amplitude_397
    ...
    def update_progress(self, iteration):
        progress = self.min_progress + (self.max_progress - self.min_progress) * float(iteration + 1.0) / len(self.scan_pwr)
...
if __name__ == '__main__':
    ...
    exprt = ramsey_with_heating_and_cryst_scan_blue(cxn = cxn)
