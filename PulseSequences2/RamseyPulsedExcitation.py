...
class RamseyPulsedExcitation(pulse_sequence):
    
    name = 'RamseyPulsedExcitation'
                            #(self, scan_param, minim, maxim, steps, unit)
    scannable_params = {
        'Motion_Analysis.scan_ramsey_time':  (0, 2, 30, 'ms')
              }

    show_params= ['Motion_Analysis.amplitude_397',
                  'Motion_Analysis.excitation_enable',
                  'Motion_Analysis.pulse_width_397',
                  'Motion_Analysis.ramsey_detuning_397',
    ...
    def sequence(self):
        ...
        ## calculate the modulation freq
        freq_397_mod=Motion_Analysis.scan_frequency
        ...
        #self.addSequence(EmptySequence, self.Heating.background_heating_time)
        
        if self.parameters.Motion_Analysis.excitation_enable:
            self.addSequence(motion_analysis)
            self.addSequence(EmptySequence,  { "EmptySequence.empty_sequence_duration" : self.parameters.Motion_Analysis.scan_ramsey_time})
            self.addSequence(motion_analysis)
            self.addSequence(OpticalPumping)
